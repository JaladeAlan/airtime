...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Extract data
    $old_pin= $utility_class_call::inputData($data, 'old_pin');
    $pin= $utility_class_call::inputData($data, 'new_pin');
    $confirm_pin = $utility_class_call::inputData($data, 'confirm_pin');

        // Validate input data
    if ($utility_class_call::validate_input($pin) || $utility_class_call::validate_input($old_pin) || 
    ...
    // Verify password
    if ($old_pin != $user_id["transaction_Pin"]){
        $text = $api_response_class_call::$incorrectPin;
        $errorcode = $api_error_code_class_call::$internalUserWarning;
        $maindata = [];
        $hint = ["Old Pin is incorrect"];
        $linktosolve = "https://";
        $api_status_code_class_call->respondBadRequest($maindata,$text,$hint,$linktosolve,$errorcode);
    }
